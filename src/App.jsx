...
function App() {
  ...
  function moveExerciseUp(index) {
    if (index === 0) return
    const n = [...exercises]; [n[index - 1], n[index]] = [n[index], n[index - 1]]; setExercises(n)
  }
  ...
  function removeExercise(index) {
    const n = [...exercises]; n.splice(index, 1); setExercises(n)
  }
  ...
  function confirmFinish(updateAll) {
    const workout = { date: new Date().toLocaleDateString('en-GB'), exercises }
  ...
  function saveEditedTemplate() {
    ...
    setFolders(nf); setExercises([]); setEditingTemplate(null)
  }

  function cancelEditTemplate() { setExercises([]); setEditingTemplate(null) }
  ...
  function getBestSet(name) {
    let best = null
    for (const w of history) for (const ex of w.exercises) if (ex.name === name) for (const s of ex.sets) {
                ...
                <div>
                  ...
                  {history.slice(0, 10).map((w, i) => (
                    <div key={i} className="bg-[#13132A] border border-[#232340] rounded-xl p-4 mb-3">
                      <div className="flex justify-between items-center mb-2"><span className="text-sm font-bold">{w.date}</span><span className="text-xs text-[#555]">{w.exercises.length} exercises</span></div>
                          ...
                          {folder.templates.length === 0 ? (
                            ...
                            <div key={ti} className="bg-[#13132A] border border-[#232340] rounded-xl p-3.5 mb-2">
                              ...
                              <div className="flex gap-2 mt-2.5">
                                <button onClick={() => loadTemplate(t)} className="flex-1 py-2 bg-[#7B7BFF] rounded-lg text-xs font-bold hover:bg-[#6060DD] transition-colors">Start</button>
